...
@app.post("/api/transcribe-vocal-audio")
async def transcribe_vocal_audio(file: UploadFile = File(...)):
    """Receives complete recorded vocal audio blob from browser MediaRecorder and runs
    Full-Buffer Monophonic Vocal Transcription with high-precision pitch, beat, and duration.
    """
    try:
        temp_path = "temp_repeater_vocal.wav"
        ...
        try:
            ...
            # Fallback load with librosa if webm format
            data, samplerate = librosa.load(temp_path, sr=16000)
        ...
        # Full Waveform Normalization
        max_amp = np.max(np.abs(data))
        if max_amp > 0:
            data = data / max_amp

        # Process unbroken audio signal with 10ms frame hop size (160 samples @ 16kHz)
        hop_size = 160
        frame_size = 512
        ...
        for i in range(max(0, num_frames)):
            ...
            time_ms = int((start_sample / sr) * 1000.0)
            rms_val = float(np.sqrt(np.mean(chunk**2)))
            
            freq, confidence = detect_pitch_with_confidence(chunk, sr)
            voiced = confidence >= 0.20 and freq >= 75.0 and freq <= 850.0
        ...
        # Run Monophonic Onset-Based Note Segmentation Engine over full unbroken recording
        sequence = segment_vocal_melody_frames(frame_list, min_note_dur_ms=100)
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Gagal melakukan transkripsi vokal: {e}")
